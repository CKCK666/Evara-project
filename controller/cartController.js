  ...
  try {
    if (req.session.user.pkUserId) {
      ...
      let productFind = await Product.find(
        {
          pkProductId: new ObjectId(req.body.pkProductId),
          strStatus: 'Active',
        },
      ...
      if (product && product.length) {
        ...
        if (userCart && userCart.length) {
          let productInCart = await Cart.find({
            pkUserId,
          ...
          if (productInCart.length) {
            ...
            let message =
              availableStock < 10
                ...
                : 'Purchase limit exceeded. Maximum allowed quantity is 10.';
            let quantityInCart = productInCart[0].arrProducts.find((pro) =>
              pro.pkProductId.equals(pkProductId)
            );
            ...
            if (quantityInCart.intQuantity < limit) {
              ...
              if (result.modifiedCount > 0) {
                ...
                if (
                  totalPriceResult.length > 0 &&
                  totalPriceResult[0].total_cart_price !== undefined
                ) {
                    ...
                    { pkUserId, strStatus: 'Active' },
                    {
                      $set: {
                        total_cart_price: parseFloat(
                          totalPriceResult[0].total_cart_price
                        ),
                      },
                    }
                  );
                  ...
                  console.log(
                    'No active products found or total_cart_price is undefined.'
                  );
                }
            ...
            if (result.modifiedCount > 0) {
              ...
              res.json({
                success: false,
                message: 'Fail to add new product in cart',
              });
            }
          }
        } else {
...
const getCartPage = async (req, res) => {
  try {
    ...
    let cartDetailsFind = await Cart.find({
      pkUserId: new ObjectId(pkUserId),
      strStatus: 'Active',
    }).populate('arrProducts.offer');
    let cartDetails = cartDetailsFind.map((cart) => {
      let filteredProducts = cart.arrProducts.filter(
        (product) => product.strStatus === 'Active'
      );
      return {
        ...cart._doc,
    ...
    if (
      cartDetails &&
      cartDetails.length &&
      cartDetails[0].arrProducts &&
      cartDetails[0].arrProducts.length > 0
    ) {
      let cartProducts = cartDetails[0].arrProducts.map((obj) => {
        ...
        return {
          ...
          pkCartId: cartDetails[0].pkCartId,
        };
      });
      ...
      let totalDiscountValue =
        totalDiscount && totalDiscount[0] ? totalDiscount[0].total_discount : 0;
      ...
      res.render('user/cartPage', {
        ...
        message: 'successfully loaded cart page',
        user: true,
        cartCount,
      });
    } else {
...
const changeQuantity = async (req, res) => {
  try {
    ...
    let productInCartFind = await Cart.find({
      pkUserId,
      'arrProducts.pkProductId': pkProductId,
      strStatus: 'Active',
    });
    ...
    if (productInCart.length) {
      ...
      if (result.modifiedCount > 0) {
        ...
        let subTotalCart = parseFloat(
          totalPriceResult[0].total_cart_price + totalDiscount[0].total_discount
        );
        res.json({
          success: true,
          message: 'cart updated-1',
          quantity,
          totalPriceResult: totalPriceResult[0].total_cart_price,
          pkUserId,
          totalDiscount: totalDiscount[0].total_discount,
          subTotalCart,
        });
      } else {
    ...
    if (req.body.pkCartId || req.body.pkProductId) {
      let findProductPrice = await Cart.aggregate([
...

        {
          $project: {
            totalPrice: {
              $multiply: ['$arrProducts.intPrice', '$arrProducts.intQuantity'],
            },
          },
        },
        {
      ...
      if (removeProduct.modifiedCount > 0) {
        let arrProductsCount = await Cart.find({
          pkCartId: new ObjectId(req.body.pkCartId),
          strStatus: 'Active',
          arrProducts: { $exists: true, $not: { $size: 0 } },
        });
        console.log('arrProductCount:', arrProductsCount);
        if (!arrProductsCount.length) {
          let findCart = await Cart.updateOne(match, {
            $set: { strStatus: 'Deleted' },
          });
          if (findCart.modifiedCount > 0) {
            return res.json({
              success: true,
              message: 'Successfully removed from cart1',
            });
          } else {
            return res.json({
              success: false,
              message: 'Failed remove from cart1',
            });
          }
        }
        let decreProdPrice = await Cart.updateOne(match, {
          $inc: { total_cart_price: -productPrice },
        });
        if (decreProdPrice.modifiedCount > 0) {
          res.json({
            success: true,
            message: 'Successfully updated total cart price',
          });
        } else {
          res.json({
            success: false,
            message: 'Failed  updated total cart price',
          });
        }
      } else {
