...
const getAddressPage = async (req, res) => {
  try {
    ...
    let addressCount = await Address.countDocuments({
    ...
    if (req.body.pkUserId) {
      ...
      if (
        ...
        if (arrAddressExist.length > 0) {
          ...
          await Address.updateOne(matchQuery2, {
    ...
    if (req.body.pkUserId) {
      if (req.body.pkAddressId) {
        ...
        if (
          ...
          if (arrAddressExist.length > 0) {
            await Address.updateMany(matchQuery, {
              $set: { isDefaultAddress: false },
            });
            isDefaultAddress = {
              ...isDefaultAddress,
    ...
    if (req.body.pkUserId) {
      if (req.body.pkAddressId) {
        ...
        if (!result.modifiedCount) {
          return res.json({
            success: false,
            message: 'Failed change to false',
          });
        }
        ...
        if (setToDefault.modifiedCount > 0) {
          console.log(
            `${result.modifiedCount} document updated to set isDefaultAddress to true for pkAddressId: ${pkAddressId}.`
          );
          res.json({
            success: true,
          ...
          res.json({
            success: false,
            message: 'Failed set as default address',
          });
        }
      } else {
        res.json({ success: false, message: 'Address id not found' });
